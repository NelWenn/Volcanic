...
public class VOptionScreen extends Screen {
    ...
    private void buildPage() {
        ...
        for (var modEntry : this.modSettingsEntries) {
            ...
            for (OptionPage page : pages) {
                ...
                VButtonWidget widget = new VButtonWidget(x, y, width, VGuiConstants.WIDGET_HEIGHT, Component.nullToEmpty(page.name), button -> { Options.shaderNav = null; Options.shaderCat = null; this.setOptionList(finalIdx); });
    ...
    private void applyOptions() {
        ...
        // presets can rewrite many settings at once, so rebuild all pages
    ...
    // in-place swap of the Shaders page blocks to match nav state, keeps pending edits
    private void refreshShaderPage() {
        int idx = pageIndexByName(Component.translatable("vulkanmod.options.pages.shaders").getString());
        if (idx < 0) {
            return;
        }
    ...
    private int pageIndexByName(String name) {
        for (int i = 0; i < this.optionPages.size(); i++) {
            if (this.optionPages.get(i).name.equals(name)) {
                return i;
            }
        }
        return -1;
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        // ESC steps back one shader nav level before closing the screen
        if (keyCode == GLFW.GLFW_KEY_ESCAPE && !this.isSearchActive) {
            if (Options.shaderCat != null) {
                Options.shaderCat = null;
                this.refreshShaderPage();
                return true;
            }
            if (Options.shaderNav != null) {
                Options.shaderNav = null;
                Options.shaderCat = null;
                this.refreshShaderPage();
                return true;
            }
