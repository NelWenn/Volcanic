...
public class Renderer {
    ...
    public void beginFrame() {
        ...
        if (FrameTimer.instance() != null) {
            FrameTimer.instance().onBeginFrame(currentFrame);
            frameTimestampArmed = true;
        }
    ...
    private void beginRenderPass(MemoryStack stack) {
        ...
        // must be outside a render pass; guard against flushCmds() re-entry moving it mid-frame
        if (frameTimestampArmed && FrameTimer.instance() != null) {
            FrameTimer.instance().cmdBeginTimestamp(commandBuffer, currentFrame);
    ...
    public void preInitFrame() {
        ...
        long _t = FrameTimer.instance() != null ? System.nanoTime() : 0;
        WorldRenderer.getInstance().uploadSections();
        UploadManager.INSTANCE.submitUploads();
        if (FrameTimer.instance() != null) FrameTimer.instance().addUploadNanos(System.nanoTime() - _t);
    }
    ...
    private void resetDescriptors() {
        ...
        // force re-set on first pipeline bind of the new frame
    ...
    public static void clearAttachments(int v, int width, int height) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (clearDepth) {
                // snapshot world depth before the clear wipes it
                INSTANCE.mainPass.onDepthClear(framebuffer);
                // pass may want the depth kept for the resolve to sample; color still cleared
                if (INSTANCE.mainPass.suppressDepthClear(framebuffer)) {
                    clearDepth = false;
                    clearDepthStencil = clearStencil;
                }
            }
