...
public class Vulkan {
    ...
    public static final boolean ENABLE_VALIDATION_LAYERS = MoltenVKConfig.validationEnabled();
    ...
    public static void initVulkan(long window) {
        ...
        // macOS: tune MoltenVK before the first Vulkan call
    ...
    private static void createInstance() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            // loader path (validation): enumerate the portability driver
            if (MoltenVKConfig.validationEnabled()) {
                createInfo.flags(createInfo.flags() | org.lwjgl.vulkan.KHRPortabilityEnumeration.VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR);
            }
    ...
    private static void createSurface(long handle) {
        ...
        try (MemoryStack stack = stackPush()) {
            surface = VkSurfaceUtil.createSurface(instance, window, stack);
        }
    }
    ...
    private static PointerBuffer getRequiredInstanceExtensions() {
        return VkSurfaceUtil.getRequiredInstanceExtensions(stackGet(), ENABLE_VALIDATION_LAYERS);
    }
